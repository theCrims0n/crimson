...
export const Chat2 = () => {
    ...
    const { users, getUsers, reconnect, lastMessages, setLastMessages, reset, message } = useChatStore2()
            ...
            <div className="relative flex flex-row justify-center items-center w-full h-full">
                <div className="flex justify-center h-full w-full absolute">
                    <div className="flex flex-row w-[1400px] relative overflow-auto" >
                        <div className="flex flex-col h-full w-full absolute pl-[50px]" onClick={() => [setUserList(false), setChatList(false)]}>
                            <ChatForm />
                        </div>
                        <div className={cn("flex flex-col h-full", userList || chatList ? 'w-[500px]' : 'w-[0px] animation duration-500')}>
                            <div className="flex flex-row h-full relative ">
                                <div className="flex flex-col h-full relative [&>*]:p-4 justify-start items-center h-full bg-black space-y-4 pt-4 border border-r-0 border-l-zinc-600 border-t-zinc-600 border-b-zinc-600 w-[10%] min-w-[50px]">
                                    <div id="chats" onClick={() => [setUserList(false), setChatList(!chatList)]} className="flex rounded-full w-10 h-10 md:w-11 md:h-11 justify-center items-center hover:bg-zinc-900 ">
                                    ...
                                    <div id="users" onClick={() => [setChatList(false), setUserList(!userList)]} className="flex rounded-full w-10 h-10 md:w-11 md:h-11 justify-center items-center hover:bg-zinc-900">
                                ...
                                <div className="flex flex-col h-full relative overflow-auto overflow-hidden w-[90%] border border-t-zinc-600 border-b-zinc-600 border-l-0 border-r-0" >
                                    <div onClick={() => setTimeout(() => [setChatList(false), setUserList(false)], 50)} className={cn("flex flex-col bg-black min-h-full min-w-full top-0 left-0 ease-in-out animation duration-300 -translate-x-full border border-r-zinc-600 border-l-0 border-t-0 border-b-0", (chatList || userList) && 'translate-x-0 ')}>
                                        {chatList ? <ChatList chats={lastMessages} /> : <ChatUsers users={users || []} />}
                                    </div>
                                </div>
                            </div>
                        </div>
