...
export const ChatUsers = ({ users }: Props) => {
    ...
    return (
        <div className="flex flex-col h-full w-full rounded-sx ">
            <div className={cn("flex flex-col w-full overflow-auto h-full ", isLoading && 'pointer-events-none animate-pulse')}>
                <ChatSearch type={1} />
                {users.length > 0 && <ul className="flex flex-col w-full">
                    {
                        users.sort((a, b) => a.email > b.email ? 1 : -1).filter(f => f._id != user._id).map((user, index) => {
                            return (
                                <li title={user.socket_id != '' ? 'Online' : 'Offline'} key={index} onClick={() => user.chat_id ? (current_chat_id != user.chat_id && getAllMessages(user.chat_id, user.socket_id, user._id)) : socketTo(user.socket_id, user._id)} className=" cursor-pointer flex hover:pl-1 border border-r-0 border-l-0 border-t-0 border-b-zinc-900 justify-start items-center hover:bg-zinc-800 w-full h-16 animation duration-300">
